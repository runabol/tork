...
func (w *Worker) taskHandler(threadname string) func(ctx context.Context, t *task.Task) error {
	return func(ctx context.Context, t *task.Task) error {
		...
		switch t.State {
		case task.Scheduled:
			return w.handleScheduledTask(ctx, t)
		case task.Cancelled:
...
func (w *Worker) handleScheduledTask(ctx context.Context, t *task.Task) error {
	started := time.Now()
	t.StartedAt = &started
...
func (w *Worker) Start() error {
	...
	for qname, concurrency := range w.queues {
		...
		for i := 0; i < concurrency; i++ {
			err := w.broker.SubscribeForTasks(qname, w.taskHandler(fmt.Sprintf("%s-%d", qname, i)))
			if err != nil {
