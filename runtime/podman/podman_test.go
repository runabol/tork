...
func TestPodmanRunTaskSandbox(t *testing.T) {
	t.Run("ubuntu", func(t *testing.T) {
		rt := NewPodmanRuntime(WithSandbox(true))
		...
		err := rt.Run(context.Background(), tk)
		assert.NoError(t, err)
		assert.Contains(t, tk.Result, "uid=1000")
		assert.Contains(t, tk.Result, "gid=1000")
	})
	...
	t.Run("no sandbox", func(t *testing.T) {
		rt := NewPodmanRuntime(WithSandbox(false))
