...
func TestMiddlewareBefore(t *testing.T) {
	order := 1
	h := func(ctx context.Context, j *tork.Job) error {
	...
	assert.NoError(t, hm(context.Background(), &tork.Job{}))
}
...
func TestMiddlewareError(t *testing.T) {
	...
	hm := ApplyMiddleware(h, []MiddlewareFunc{mw1, mw2})
	assert.ErrorIs(t, hm(context.Background(), &tork.Job{}), Err)
}
