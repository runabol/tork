...
func TestMiddlewareBefore(t *testing.T) {
	order := 1
	h := func(ctx context.Context, tk *tork.Task) error {
		assert.Equal(t, 3, order)
		return nil
	}
	mw1 := func(next HandlerFunc) HandlerFunc {
		return func(ctx context.Context, tk *tork.Task) error {
	...
	assert.NoError(t, hm(context.Background(), &tork.Task{}))
}
...
func TestMiddlewareError(t *testing.T) {
	...
	hm := ApplyMiddleware(h, []MiddlewareFunc{mw1, mw2})
	assert.ErrorIs(t, hm(context.Background(), &tork.Task{}), Err)
}
